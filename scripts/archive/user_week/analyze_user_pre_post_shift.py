...
Archive (`comment_features`): single `ai_likeness_user_week` composite.
...
- Default (both strict and loose cohorts):
  `.venv/bin/python scripts/user_week/analyze_user_pre_post_shift.py --config config/archive/ai_adoption_political_forums_setup.yaml`
- Strict only with explicit thresholds:
  `.venv/bin/python scripts/user_week/analyze_user_pre_post_shift.py --config config/archive/ai_adoption_political_forums_setup.yaml --cohort strict --min_words_per_week 100 --min_pre_weeks 4 --min_post_weeks 4 --min_total_words_pre 400 --min_total_words_post 400`
"""
...
ITALY_FEATURE_SPECS: Dict[str, Dict[str, str]] = {
...
ITALY_DEFAULT_FEATURES: List[str] = [
    "ai_style_rate_100w",
    "semicolon_rate_100w",
    "em_dash_rate_100w",
    "hedging_phrase_rate_100w",
...
ITALY_COMPOSITE_COMPONENTS: List[Tuple[str, int]] = [
    ("ai_style_rate_100w", +1),
    ("semicolon_rate_100w", +1),
    ("em_dash_rate_100w", +1),
    ("hedging_phrase_rate_100w", +1),
]
ITALY_POLARIZATION_COMPOSITE_COMPONENTS: List[Tuple[str, int]] = [
    ("extremity", +1),
    ("net_ideology", +1),
    ("other_side_salience_rate_100w", +1),
    ("aggression_rate_100w", +1),
]
ITALY_COMPOSITE_NAME = "ai_style_composite_user_week"
ITALY_POLARIZATION_COMPOSITE_NAME = "polarization_composite_user_week"
...
def study_analysis_context(
    ...
    if infer_user_week_input_mode(config) == "enriched_shards":
        specs.update(ITALY_FEATURE_SPECS)
        yaml_feats = user_week_default_features(config)
        defaults = yaml_feats if yaml_feats else list(ITALY_DEFAULT_FEATURES)
        yaml_composites = user_week_composites(config)
        if yaml_composites:
            composites = yaml_composites
        else:
            composites = [
                {"name": ITALY_POLARIZATION_COMPOSITE_NAME, "components": list(ITALY_POLARIZATION_COMPOSITE_COMPONENTS)},
                {"name": ITALY_COMPOSITE_NAME, "components": list(ITALY_COMPOSITE_COMPONENTS)},
            ]
    else:
        composites = [{"name": COMPOSITE_NAME, "components": list(COMPOSITE_COMPONENTS)}]
...
def main() -> None:
    ...
    if not paths.panel_path.exists():
        raise FileNotFoundError(
            f"User-week panel not found: {paths.panel_path}. Run scripts/user_week/prepare_user_week_style_panel.py first."
        )
...
def write_methods_note(path: Path, ban_iso_str: str) -> None:
    ...
    lines = [
        ...
        "",
        "Composites (Italy enriched_shards; see config user_week for weights):",
        ...
        "Archive comment_features mode uses ai_likeness_user_week (legacy AI-adoption stack).",
