...
from src.config_utils import load_config, subreddit_topic_map, utc_ts

LAUNCH_DATE_UTC = pd.Timestamp(datetime(2022, 11, 30))
WORD_WEIGHT_COLS = [
    "semicolon_rate_100w",
    "semicolon_extended_rate_100w",
    "ai_word_rate_100w",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--topic_views",
        action="store_true",
        help="Generate topic-level figures (daily/weekly/trailing-rolling) from by-subreddit table.",
    )
    parser.add_argument(
...
def aggregate_topic_daily(df_by_sub: pd.DataFrame, subreddit_to_topic: dict[str, str]) -> pd.DataFrame:
    """Function summary: aggregate per-subreddit daily rows into per-topic daily rows using config-driven mapping."""
    if df_by_sub.empty:
        return pd.DataFrame()
...
def main() -> None:
    ...
    config_topic_map = subreddit_topic_map(config)
    ...
    pooled_specs: list[tuple[str, str, str]] = [
        ...
        ("quote_curly_share", "Event-time: Curly Quote Share (curly / all quotes)", "event_time_quote_curly_share.png"),
    ...
    for view_name, view_df in pooled_views:
        ...
        plot_two_series_same_axes(
            ...
            "semicolon strict",
            "semicolon extended",
            f"Event-time: Strict vs Extended Semicolon Signal (pooled, {view_name})",
            "Rate per 100 words",
            view_dir / "event_time_semicolon_strict_vs_extended.png",
            event_time_xlabel_text=xt,
            show_markers=show_markers,
        )
        plot_two_series_same_axes(
        ...
        plot_two_series_same_axes(
            ...
            "quote_all_rate_100w",
            "curly quote strict",
            "all quote chars extended",
            f"Event-time: Strict vs Extended Quote Signal (pooled, {view_name})",
            "Rate per 100 words",
            view_dir / "event_time_quote_strict_vs_extended.png",
            event_time_xlabel_text=xt,
            show_markers=show_markers,
        )
        plot_two_series_same_axes(
            ...
            "time_expression_rate_100w",
            "url rate",
            "time-expression rate",
            f"Event-time: URL vs Time-expression Rates (pooled, {view_name})",
            "Rate per 100 words",
            view_dir / "event_time_url_vs_time_expression_rate.png",
            event_time_xlabel_text=xt,
            show_markers=show_markers,
        )
    ...
    if not df_by_sub.empty:
        by_sub_specs: list[tuple[str, str, str]] = [
            ...
            (
                "semicolon_extended_rate_100w",
                "Per-subreddit: Extended Semicolon Rate (per 100 words)",
                "event_time_semicolon_extended_rate.png",
            ),
        ...
        for view_name, view_df in by_sub_views:
            ...
            for y_col, title, fname in by_sub_specs:
                if y_col in view_df.columns:
                    if not bool(pd.to_numeric(view_df[y_col], errors="coerce").notna().any()):
                        print(
                            f"[plot_event_time_metrics] by_subreddit_metric_skip_all_nan view={view_name} metric={y_col}",
                            flush=True,
                        )
                        continue
    ...
    if args.topic_views and not df_by_sub.empty:
        topic_specs: list[tuple[str, str, str]] = [
            ...
            (
                "semicolon_extended_rate_100w",
                "Per-topic ({view}): Extended Semicolon Rate (per 100 words)",
                "event_time_semicolon_extended_rate.png",
            ),
            ("em_dash_rate_100w", "Per-topic ({view}): Em Dash Rate (per 100 words)", "event_time_em_dash_rate.png"),
            (
                "em_dash_extended_rate_100w",
                "Per-topic ({view}): Extended Dash Rate (per 100 words)",
                "event_time_em_dash_extended_rate.png",
            ),
            ...
            (
                "colon_extended_rate_100w",
                "Per-topic ({view}): Extended Colon Rate (per 100 words)",
                "event_time_colon_extended_rate.png",
            ),
            ...
            ("quote_all_rate_100w", "Per-topic ({view}): All Quote Characters (per 100 words)", "event_time_quote_all_rate.png"),
            ("quote_curly_share", "Per-topic ({view}): Curly Quote Share", "event_time_quote_curly_share.png"),
            ("url_rate_100w", "Per-topic ({view}): URL Rate (per 100 words)", "event_time_url_rate.png"),
            (
                "time_expression_rate_100w",
                "Per-topic ({view}): Time-expression Rate (per 100 words)",
                "event_time_time_expression_rate.png",
            ),
            (
                "markdown_bold_pair_rate_100w",
            ...
            ("ai_word_rate_100w", "Per-topic ({view}): Strict Top-10 Stem-aware Rate (per 100 words)", "event_time_ai_word_rate.png"),
            (
                "ai_word_extended_rate_100w",
        ...
        topic_metric_cols = [m[0] for m in topic_specs]
        topic_daily = aggregate_topic_daily(df_by_sub, config_topic_map)
        topic_weekly = aggregate_topic_weekly(topic_daily)
        topic_rolling_daily = grouped_trailing_daily_rolling(
        ...
        for view_name, view_df in topic_views:
            ...
            coverage_rows.extend(
                collect_metric_coverage_rows(
                    ...
                    metric_cols=topic_metric_cols,
                )
            )
            view_df = apply_metric_coverage_gate(view_df, topic_metric_cols, min_metric_coverage=float(args.min_metric_coverage))
            show_markers = view_name != "rolling_daily"
            ...
            for y_col, title_template, filename_template in topic_specs:
                if y_col in view_df.columns:
                    if not bool(pd.to_numeric(view_df[y_col], errors="coerce").notna().any()):
                        print(f"[plot_event_time_metrics] topic_metric_skip_all_nan view={view_name} metric={y_col}", flush=True)
                        continue
