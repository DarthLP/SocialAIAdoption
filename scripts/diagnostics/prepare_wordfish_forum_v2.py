"""
Script summary:
Forum-level Wordfish v2 — shard topic_family labels, token cap, alternating MLE.

Functionality:
- Same four fits as legacy 03 (it/en × day/week); does NOT overwrite enrich topic_family.
- Writes to wordfish_forum_v2/ (legacy wordfish/ untouched).
- Forum-level validation reported; theta NOT validated as ideology (gate expected to fail).
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Prepare forum Wordfish v2 tables.")
...
def build_documents(
    ...
    use_shard = bool(wf_cfg.get("use_shard_topic_family", True)) and "topic_family" in work.columns
    if use_shard:
        work["topic_family"] = work["topic_family"].fillna("").astype(str)
    else:
    ...
    grouped = work.groupby(["subreddit", "bin_start"], dropna=False)
    docs: List[DocumentRecord] = []
    coverage_rows: List[Dict[str, Any]] = []

    max_tok = int(wf_cfg.get("max_tokens_per_doc", 0))
    seed = int(wf_cfg.get("token_subsample_seed", 42))

    for (sub, bin_start), grp in grouped:
        family = str(grp["topic_family"].mode().iloc[0]) if len(grp) else ""
...
def forum_validation_gate(
    validation: pd.DataFrame,
    wf_cfg: Dict[str, Any],
) -> pd.DataFrame:
    """Function summary: forum-level validation gate (expected to fail for ideology use).

    Parameters:
    - validation: per-language/time_bin correlation rows.
    - wf_cfg: config with validation.*.

    Returns:
    - Gate summary dataframe.
    """
    val_cfg = wf_cfg.get("validation") or {}
    threshold = float(val_cfg.get("gate_abs_rho_sem_axis", 0.5))
    min_subs = int(val_cfg.get("min_subreddits", 5))
    expect_fail = bool(val_cfg.get("expect_gate_fail", True))
    rows: List[Dict[str, Any]] = []
    for _, row in validation.iterrows():
        lang = row.get("primary_lexicon")
        if lang != "it":
            continue
        n = int(row.get("n_subreddits", 0))
        rho_sem = row.get("spearman_theta_sem_axis")
        abs_sem = abs(float(rho_sem)) if rho_sem == rho_sem else float("nan")
        passed = (n >= min_subs) and (abs_sem >= threshold)
        rows.append(
            {
                ...
                "expect_gate_fail": int(expect_fail),
                "recommendation": (
                    "do_not_use_forum_theta_as_ideology"
                    if expect_fail or not passed
                    else "forum_theta_passed_gate_unexpected"
                ),
            }
        )
    return pd.DataFrame(rows)
...
def main() -> None:
    ...
    wf_cfg = load_wordfish_forum_v2_config(config)
    subdir = str(wf_cfg.get("output_tables_subdir", "wordfish_forum_v2"))
    ...
    if "topic_family" not in raw.columns or not wf_cfg.get("use_shard_topic_family", True):
    ...
    run_notes: List[str] = [
        "Wordfish forum v2 run notes",
        "=========================",
        wf_cfg.get("note", ""),
        "Forum theta is NOT validated as an ideology measure (gate expected to fail).",
        "Use for Tier B / family dispersion / DDD panels with matched time_bin only.",
        "EN fits mix US political subs with ukpolitics — axis confounds country vs ideology.",
        "",
    ...
    for lang in langs:
        ...
        for time_bin in time_bins:
            ...
            run_notes.append(
                ...
                f"sign_flipped={result.sign_flipped} one_sided_beta={result.one_sided_beta}{lr_note}"
            )
    ...
    if all_pos:
        ...
        if not val.empty:
            ...
            gate = forum_validation_gate(val, wf_cfg)
            if not gate.empty:
                gate.to_csv(out_dir / "wordfish_validation_gate.csv", index=False)
                for _, g in gate.iterrows():
                    run_notes.append(
                        f"GATE IT/{g['time_bin']}: pass={g['gate_pass']} "
                        f"|rho_sem|={g['abs_spearman_theta_sem_axis']:.4f} -> {g['recommendation']}"
                    )
    ...
    print(f"[prepare_wordfish_forum_v2] wrote tables to {out_dir}", flush=True)
