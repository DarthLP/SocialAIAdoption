...
def main() -> None:
    ...
    subdir = str(wf_cfg.get("output_figures_subdir", "wordfish_forum_v2"))
    tbl = tables_subdir(config, subdir)
    ...
    if not disp_path.is_file() or not pos_path.is_file():
        print("[plot_wordfish_forum_v2] missing prepared tables — run prepare_wordfish_forum_v2.py first", flush=True)
        return
    ...
    print(f"[plot_wordfish_forum_v2] wrote figures to {fig_dir}", flush=True)
