...
def _plot_panel_bundle(
    ...
    *,
    ...
    if pos_path is not None and ideol_path.is_file():
        ...
        merged_all = auth_theta.merge(ideol, on="author", how="inner", suffixes=("", "_ideol"))
...
def main() -> None:
    ...
    wfa = load_wordfish_authors_config(config)
    tab_dir = tables_subdir(config, "wordfish_authors")
    fig_dir = figures_subdir(config, "wordfish_authors")
