...
`scripts/analyze_user_pre_post_shift.py` and the user-week panel produced by
`scripts/prepare_user_week_style_panel.py`, and writes a small set of figures
...
- One cohort only:
  `.venv/bin/python scripts/plot_user_pre_post_shift.py --config config/political_forums_setup.yaml --cohort strict`
"""
