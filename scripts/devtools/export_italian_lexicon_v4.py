"""
Script summary:
Export curated italian_political_lexicon_v4.csv into archived config/archive/lexicons/ snapshots (not runtime).

Functionality:
- Merges v4 Italian lemmas into ideology_it.txt (dominant L/C/R default), issue_it.txt,
  other_side_it.txt, pairs_it.json, stance/valence/polarized lexicons, term_meta_it.json.
  Political salience uses data/raw/political_lexicon_parallel.csv (not flat political_*.txt).
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Export italian_political_lexicon_v4 to config/archive/lexicons/ snapshots."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Archive root (default: config/archive/lexicons)",
    )
    parser.add_argument(
        "--policy",
...
def export_lexicons(
    ...
    - output_dir: archive lexicon root (default config/archive/lexicons).

    Returns:
    - Summary dict with counts.
    """
    archive_root = output_dir or (root / "config" / "archive" / "lexicons")
    categorized_dir = archive_root / "categorized"
    v4_dir = archive_root / "v4_export"
    categorized_dir.mkdir(parents=True, exist_ok=True)
    v4_dir.mkdir(parents=True, exist_ok=True)
    tables_dir = root / "results" / "tables" / "italy_polarization"
    tables_dir.mkdir(parents=True, exist_ok=True)

    ideology_path = categorized_dir / "ideology_it.txt"
    issue_path = categorized_dir / "issue_it.txt"
    other_side_path = categorized_dir / "other_side_it.txt"
    ...
    if not dry_run:
        ...
        if policy == "dominant":
            _write_pairs_json(v4_dir / "pairs_it.json", pairs_out, input_path)
            _write_term_meta_json(v4_dir / "term_meta_it.json", term_meta, input_path)
            _write_categorized_flat(v4_dir / "stance_it.txt", stance_by_cat, "stance", input_path, policy)
            _write_categorized_flat(v4_dir / "valence_it.txt", valence_by_cat, "valence", input_path, policy)
            _write_categorized_flat(v4_dir / "polarized_it.txt", polarized_by_cat, "polarized", input_path, policy)
            stamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
            (v4_dir / "dominant_export_stamp.txt").write_text(
