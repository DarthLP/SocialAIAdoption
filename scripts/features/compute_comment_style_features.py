"""
Script summary:
Add punctuation/markdown style counts and phrase-lexicon hits to enriched Italy Parquet shards.

Functionality:
- Thin wrapper around `_enriched_shard_runner.py` (--pass style).

How to apply/run:
  .venv/bin/python scripts/features/compute_comment_style_features.py --config config/italy_polarization_setup.yaml
  .venv/bin/python scripts/features/compute_comment_style_features.py --config config/italy_polarization_setup.yaml --subreddit politicaITA --max-shards 1
"""
...
if __name__ == "__main__":
    _load_runner().main_with_pass(
        fixed_pass="style",
        prog="compute_comment_style_features",
