...
multiple hues: old vs new vs new_first_comment users; short vs medium vs long length buckets;
...
USER_SERIES_LABELS = {
    ...
    "new_first_comment": "New users (debut comment only)",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help="Centered rolling window size (days) for rolling_daily views.",
    )
...
def grouped_centered_daily_rolling_stratified(
    df_daily: pd.DataFrame, group_col: str, rolling_window_days: int, launch_date: pd.Timestamp
) -> pd.DataFrame:
    """Function summary: centered day rolling mean per stratum; refresh event_time_t from config launch."""
    if df_daily.empty:
        return pd.DataFrame()
...
def pooled_stratified_metric_specs() -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        ("ai_word_rate_100w", "Stratified pooled: AI-typical Word Rate (strict 10)", "stratified_event_time_ai_word_rate.png"),
        (
...
def main() -> None:
    ...
    user_roll = grouped_centered_daily_rolling_stratified(
        user_daily, "user_series", int(max(1, args.rolling_window)), launch_pd
    )
    len_roll = grouped_centered_daily_rolling_stratified(
    ...
    user_order = ["old", "new", "new_first_comment"]
