...
def build_pooled_daily(metrics_df: pd.DataFrame, ai_word_long_df: pd.DataFrame, launch_ts: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for date_utc, group in metrics_df.groupby("date_utc", sort=True):
        ...
        if "repetition_template_similarity" in group.columns:
            m_rep = group["repetition_template_similarity"].notna()
            if bool(m_rep.any()):
                rep_pooled = float(
                    group.loc[m_rep, "repetition_template_similarity"].mul(group.loc[m_rep, "n_comments"]).sum()
                    / float(group.loc[m_rep, "n_comments"].sum())
                )
        pooled_rows.append(
            {
                "subreddit": "ALL",
...
def write_notes(path: Path) -> None:
    ...
    lines = [
        ...
        "  generate with scripts/compute_daily_repetition_similarity.py. If missing, column is NaN.",
        "",
...
def merge_repetition_daily_metrics(metrics_df: pd.DataFrame, repetition_csv: Path) -> pd.DataFrame:
    ...
    if not repetition_csv.exists():
        print(
            ...
            "setting repetition_template_similarity to NaN (run compute_daily_repetition_similarity.py).",
            flush=True,
        )
